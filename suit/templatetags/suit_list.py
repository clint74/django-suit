...
def headers_handler(result_headers, cl):
    ...
    for i, header in enumerate(result_headers):
        ...
        if not isinstance(attrs, dict):
            raise TypeError(
                '"suit_column_attributes" method must return dict. '
...
def cells_handler(results, cl):
    ...
    for row, result in enumerate(results):
        ...
        for col, item in enumerate(result):
            ...
            if not isinstance(attrs, dict):
                raise TypeError(
                    '"suit_cell_attributes" method must return dict. '
                    "Got: %s: %s" % (attrs.__class__.__name__, attrs)
                )
...
def suit_list_filter_vertical(filters, cl):
    ...
    return [
        f for f in filters if not _is_horizontal(filter_horizontal, get_filter_id(f))
    ]
